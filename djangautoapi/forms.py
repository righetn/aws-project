...
class AddModelForm(forms.Form):
    brand_name = forms.CharField(
        label="Brand name", max_length=30, required=True
    )
    model_name = forms.CharField(
        label="Model name", max_length=50, required=True
    )
    ...
    price = forms.IntegerField(
        label="Price €", validators=[MinValueValidator(0)], required=True
    )

    def __init__(self, *args, **kwargs):
        super(AddModelForm, self).__init__(*args, **kwargs)
        self.fields["brand_name"].queryset = CarBrand.objects.all().order_by(
            "name"
        )
        self.fields["model_name"].queryset = CarModel.objects.all().order_by(
            "name"
        )
