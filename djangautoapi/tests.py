...
class APITestCase(TestCase):
    def setUp(self):
        ...
        self.car_model = CarModel.objects.create(
            brand=self.car_brand, name="206", production_year=1998, price=5000
        )
    ...
    def test_delete_model(self):
        ...
        car_model = CarModel.objects.create(
            brand=car_brand, name="laguna", production_year=2005, price=5000
        )
    ...
    def test_add_car(self):
        ...
        self.assertTrue(
            len(Car.objects.filter(model=self.car_model)) == nb_car + 1
        )

    def test_remove_car(self):
        car = Car.objects.create(
            model=self.car_model, price=5000, occasion=False
        )
        response = self.c.get(
            "/remove_car/" + str(self.car_model.pk) + "/" + str(car.pk)
        )
